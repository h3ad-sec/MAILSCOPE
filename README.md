...
| Origin IP | Earliest hop with a public/routable IP (falls back to oldest hop or X-Originating-IP), skipping localhost/internal relay IPs, with one-click enrich to X-VERDIKT and PHISHOPS handoff |
...
| Microsoft 365 Signals | SCL, connecting IP (CIP), country, PTR from X-Forefront-Antispam-Report, plus Microsoft's compauth composite auth verdict (conditional, shown only if M365 headers are present) |
...
| RETURN_PATH_MISMATCH | MEDIUM | Return-Path domain differs from From |
| REPLY_REDIRECT | MEDIUM | Reply-To redirects to a different domain |
| MSGID_MISMATCH | LOW | Message-ID domain differs from From |
| BRAND_IMPERSONATION | HIGH | Display name claims a known brand but the domain isn't that brand's own domain — catches unrelated domains and lookalike/combosquat domains alike (22 brands) |
| HOMOGRAPH | HIGH | From domain uses digit substitution, Unicode confusables, or a decoded punycode/IDN label to impersonate a known brand |
| IDN_DOMAIN | MEDIUM | From domain is punycode/IDN-encoded (independent of any brand match) — rare in legitimate business mail |
...
| MISSING_DATE | MEDIUM | No Date header present |
| FUTURE_DATE | MEDIUM | Date header is more than 24 hours in the future |
...
| M365_SCL_HIGH | MEDIUM/HIGH | Exchange Online Spam Confidence Level ≥5 (HIGH if ≥9) |
| M365_COMPAUTH_FAIL | HIGH | Microsoft's composite authentication (compauth) failed — independent of raw SPF/DKIM/DMARC |
...
- Brand impersonation: display name claims a brand whose domain isn't that brand's own canonical domain — catches both unrelated domains and combosquat lookalikes (e.g. `paypal-secure.net`)
- Homograph detection: digit/lookalike substitution, Unicode confusables (Cyrillic/Greek), and RFC 3492 punycode/IDN decoding, across 22 known brands
- Suspicious subject pattern matching: 15 social-engineering phrase patterns
- Date header validation: missing, future-dated (+24h), stale (30+ days)
- Origin IP resolution skips localhost/RFC1918 relay hops and reports the earliest hop with a public IP, preferring Microsoft 365's CIP (connecting IP) when present
- X-Spam headers: `X-Spam-Status`, `X-Spam-Score`, `X-Spam-Flag`
- Microsoft 365 / Exchange Online: `X-Forefront-Antispam-Report` (SCL, CIP, CTRY, PTR), `X-MS-Exchange-Organization-SCL`, and compauth from Authentication-Results
